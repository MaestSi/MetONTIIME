...
**MetONTIIME** is a Meta-barcoding pipeline for analysing ONT data in QIIME2 framework. The whole bioinformatic workflow consists of a preprocessing pipeline and a script emulating EPI2ME 16S workflow, Blasting each read against a user-defined database, so to make the whole bioinformatic analysis from raw fast5 files to taxonomy assignments straightforward and simple. Tested with Ubuntu 14.04.4 LTS.
...
* Guppy, the software for basecalling and demultiplexing provided by ONT. Tested with Guppy v3.3.
...
A conda environment named _MetONTIIME_env_ is created, where seqtk, porechop, pycoQC, NanoFilt and qiime2-2019.10 are installed.
...
* \<sampling_depth\>: number of reads subsampled for each sample for building the phylogenetic tree and for calculating diversity and rarefaction metrics; this value can be chosen looking at demu_summary.qzv or at logfile.txt
